...
```{r ans11}
loss1Companies0 <- finStatement$`營業利益（損失）` <0
loss1Companies <- c(finStatement$公司名稱[loss1Companies0])
loss1Companies
```
...
```{r ans12}
loss2Companies0 <- finStatement$`基本每股盈餘（元）` <0
loss2Companies <- c(finStatement$公司名稱[loss2Companies0])
loss2Companies
```
...
```{r ans13}
only1LossCompanies0 <-(finStatement$`營業利益（損失）` >0 & finStatement$`基本每股盈餘（元）` <0) | (finStatement$`營業利益（損失）` <0 & finStatement$`基本每股盈餘（元）` >0)

only1LossCompanies <- c(finStatement$公司名稱[only1LossCompanies0])
only1LossCompanies
...
```{r ans22}
df_oneShow <- data.frame(title = exhibitionList[[1]][[3]],
                         time = exhibitionList[[1]][[5]][[1]][[1]],
                         location = exhibitionList[[1]][[5]][[1]][[2]],
                         locationName = exhibitionList[[1]][[5]][[1]][[3]],
                         price = exhibitionList[[1]][[5]][[1]][[5]],
                         endTime = exhibitionList[[1]][[5]][[1]][[8]],
                         sourceWebName = exhibitionList[[1]][[18]],
                         stringsAsFactors = F)
df_oneShow
```
...
df_oneShow <- data.frame(title = exhibitionInput[[3]],
                         time = exhibitionInput[[5]][[1]][[1]],
                         location = exhibitionInput[[5]][[1]][[2]],
                         locationName = exhibitionInput[[5]][[1]][[3]],
                         price = exhibitionInput[[5]][[1]][[5]],
                         endTime = exhibitionInput[[5]][[1]][[8]],
                         sourceWebName = exhibitionInput[[18]],
                         stringsAsFactors = F)
...
```{r ans25}
totalCount0 <- df_allShows$sourceWebName == "全國藝文活動資訊系統"
totalCount <- sum(totalCount0)
totalCount
```
